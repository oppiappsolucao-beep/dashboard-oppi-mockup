...
.card-red { border-left: 7px solid #9B0033; }
.kpi-title { font-size: 14px; font-weight: 800; color: #334155; }
.kpi-value { font-size: 40px; font-weight: 900; color: #0f172a; line-height: 1; margin-top: 8px; }
.kpi-sub { font-size: 12px; color: #64748b; margin-top: 10px; }
</style>
    ...
    ">
        ...
        <div>
            <div style="
                ...
                color: #0f172a;
                line-height: 1;
            ">📊 Operação Comercial</div>

            <div style="
                font-size: 14px;
                color: #64748b;
                margin-top: 12px;
            ">Dashboard mockup para demonstração de automações, contratos e vendas</div>
        </div>
    </div>
</div>
""", height=130)
...
with g1:
    ...
    fig = px.bar(status_lista, x="Status", y="Quantidade", text="Quantidade")
    fig.update_layout(
        height=380,
        paper_bgcolor="white",
        plot_bgcolor="white",
...
with g2:
    ...
    fig2 = px.pie(unidade_lista, names="Unidade", values="Valor", hole=0.45)
    fig2.update_layout(
        height=380,
        paper_bgcolor="white",
        margin=dict(t=20, b=30, l=10, r=10)
